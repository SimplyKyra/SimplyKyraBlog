...
find . \( -name "*original*" -o -name "*ignore*" -o -name "*collage*" -o -name "*interest*" -o -name "*nstagram*" -o -name "*Screen Shot*" \) -delete 
...
# -geometry: my image was cropped on the sides and not bottom so I wanted it to be moved slightly to the left
# -watermark: control the opacity of the overlaid watermark. I wanted it slightly see-through
find * -exec composite -compose atop -gravity southeast -geometry +20-0 -watermark 60% -background none $watermark {} "compressed_{}" \;

# Moves all the images up a directory
mv * ..

# Removes everything in the temporary directory so I can compress the screenshots
rm -rf * 

# Copy all my Screen Shot images and pastes them in my current directory so I can compress them
find ../ -maxdepth 1 -name "*Screen Shot*" -exec cp {} . \;
mogrify -path . -resize 750 -quality 100% -define jpeg:extent=70KB * 2>/dev/null
